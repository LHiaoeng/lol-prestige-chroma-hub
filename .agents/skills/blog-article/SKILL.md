...
开始检索、拟稿或编辑前，必须完整读取仓库根目录的 [`CONTEXT.md`](../../../CONTEXT.md)。文章的中英文术语、概念边界和对象关系必须遵循该文件，并避开其中各术语的 `_Avoid_` 表述。

只有完整读取并能据此检查文章术语后，才算完成此前置步骤。若文件缺失或无法读取，暂停博客任务并向用户报告，不凭记忆补写领域定义。

## 原创常青指南

当文章回答长期稳定的玩家问题，而不是报道一次性新闻或限时活动时，按常青指南交付：

1. 写作前明确文章替读者解决的问题及本站提供的独立价值。正文至少提供一种可核验的原创价值：基于数据的分析、跨来源整理与比较、明确的方法或局限说明、能替读者节省时间的操作指南。仅翻译现有页面、改写图鉴字段或套用相同段落结构不算完成。
2. 中文与英文分别围绕各自读者的问题组织完整内容；两种语言共享事实底稿，但不是逐句翻译或其中一种语言的摘要版。
3. 使用现有常青指南维护数据和 `ArticleMaintenance` 组件，在两种语言的文章中展示作者、最后核验日期、资料来源、纠错入口和两条相关指南。来源应能直接支撑正文关键事实；由本站图鉴计算的数据应标明图鉴快照及统计口径。
4. 将最后核验日期同时用于页面的 `modifiedTime` 和 `BlogPosting.dateModified`，使可见维护信息与 SEO 元数据一致。
5. 完稿后逐项对照 `CONTEXT.md` 检查两种语言的术语、服务器层级和电竞赛区边界；所有 `_Avoid_` 表述均已排除，才算完成术语检查。
6. 在发布产物中验证中英文路由都包含作者、来源、最后核验日期、纠错入口和相关指南，并验证 `article:modified_time` 与最后核验日期一致。
...
| 常青指南维护 | `src/blog/evergreen-guides.ts` | 常青指南登记核验日期、来源和两条相关指南 |
...
- 领域术语以 `CONTEXT.md` 为唯一准绳，中英文分别使用其中定义的主称呼
...
常青指南还需在中英文构建产物中验证可见作者、来源、最后核验日期、纠错入口和两条相关指南，并验证 Open Graph 修改时间与最后核验日期一致。测试最终产物，不重复断言组件内部实现。
...
- 常青指南维护组件：`src/components/ArticleMaintenance.astro`
- 常青指南维护数据：`src/blog/evergreen-guides.ts`
