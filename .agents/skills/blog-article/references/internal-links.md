# 站内内链地图

本文件记录博客文章之间的内链关系。新增文章时更新此文件。

## 当前内链矩阵

行 = 来源页面，列 = 链向页面，✅ = 已有链接。

| 来源 ＼ 链向 | what-is-lol | chroma-skins | prestige-chromas | kaisa | champion-most | without | patch-26-15 | top-2 | summon-aug-2026 | porcelain | patch-26-16 | haibow-jax | lucky-gate | patch-26-17 | summon-sep-2026 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **what-is-league-of-legends** | — | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **what-are-chroma-skins** | ❌ | — | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **what-are-prestige-chromas** | ❌ | ❌ | — | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ | ✅ |
| **kaisa-prestige-chroma** | ❌ | ✅ | ✅ | — | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **champion-most-prestige-chromas** | ❌ | ❌ | ✅ | ❌ | — | ✅ | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **champions-without-prestige-chroma** | ❌ | ✅ | ✅ | ❌ | ❌ | — | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **patch-26-15-prestige-chromas** | ❌ | ✅ | ✅ | ❌ | ❌ | ❌ | — | ✅ | ❌ | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| **top-2-prestige-chroma-champions** | ❌ | ✅ | ✅ | ❌ | ✅ | ❌ | ❌ | — | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **prestige-chroma-summon-august-2026** | ❌ | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | — | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ |
| **blue-porcelain-prestige-chromas** | ❌ | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | — | ✅ | ❌ | ✅ | ❌ | ❌ |
| **patch-26-16-prestige-chromas** | ❌ | ✅ | ✅ | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ | ✅ | — | ❌ | ✅ | ❌ | ❌ |
| **challenger-mayhem-jax-prestige-chroma** | ❌ | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ❌ | ❌ | — | ❌ | ❌ | ❌ |
| **lucky-gate-porcelain-charm-202608** | ❌ | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ | ❌ | — | ❌ | ❌ |
| **patch-26-17-prestige-chromas** | ❌ | ❌ | ✅ | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ❌ | — | ❌ |
| **prestige-chroma-summon-september-2026** | ❌ | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | — |

新增页面 `lucky-gate-petals-of-spring-chromas-202607` 的关系：正文链向 `what-are-prestige-chromas` 与 `patch-26-15-prestige-chromas`；`patch-26-15-prestige-chromas` 和 `lucky-gate-porcelain-charm-202608` 回链本页。

新增页面 `patch-26-17-prestige-chromas` 的关系：正文链向 `what-are-prestige-chromas`、`patch-26-16-prestige-chromas`、`challenger-mayhem-jax-prestige-chroma` 与 `champion-most-prestige-chromas`，FAQ 链向 `what-are-prestige-chromas` 与 `challenger-mayhem-jax-prestige-chroma`；不修改旧文回链。

新增页面 `joy-club-peak-gala-202606` 的关系：正文链向原有 `joy-club-peak-gala-202607`、`what-are-prestige-chromas` 与 `what-are-chroma-skins`；FAQ 链向 `what-are-prestige-chromas`；原有 `joy-club-peak-gala-202607` 与 `what-are-prestige-chromas` 回链本页。

新增页面 `prestige-chroma-summon-september-2026` 的关系：正文链向 `what-are-prestige-chromas`（正文两处+FAQ）、`prestige-chroma-summon-august-2026`（往期活动表格+正文）；`prestige-chroma-summon-august-2026`（关于璀璨臻彩召唤活动段落）与 `what-are-prestige-chromas`（官方获取来源）回链本页。

2026-09-01: 新增 `full-gift-reward-prestige-chroma-202608`（满额礼赠活动资讯）。正文链向 `what-are-prestige-chromas`（正文+FAQ）与 `patch-26-15-prestige-chromas`（正文提及 1000 积分奖励出处）；`patch-26-15-prestige-chromas`（女帝菲奥娜段落）与 `joy-club-peak-gala-202607`（什么是心悦巅峰盛典段落）回链本页。

## 入链统计

| 页面 | 入链数 | 状态 |
|---|---|---|
| what-are-prestige-chromas | 8 | ✅ 枢纽 |
| what-are-chroma-skins | 3 | ✅ 良好 |
| champions-without-prestige-chroma | 1 | ⚠️ 偏少 |
| what-is-league-of-legends | 0 | ❌ 孤岛 |
| kaisa-prestige-chroma | 0 | ❌ 孤岛 |
| champion-most-prestige-chromas | 2 | ✅ 良好 |
| top-2-prestige-chroma-champions | 0 | ❌ 孤岛 |
| prestige-chroma-summon-august-2026 | 4 | ✅ 良好 |
| patch-26-15-prestige-chromas | 1 | ✅ 已连接 |
| blue-porcelain-prestige-chromas | 3 | ✅ 良好 |
| patch-26-16-prestige-chromas | 2 | ✅ 良好 |
| challenger-mayhem-jax-prestige-chroma | 0 | ❌ 新孤岛 |
| lucky-gate-porcelain-charm-202608 | 2 | ✅ 新页面 |
| lucky-gate-petals-of-spring-chromas-202607 | 2 | ✅ 新页面 |
| patch-26-17-prestige-chromas | 0 | ❌ 新孤岛（应用户要求不修改旧文回链） |
| prestige-chroma-summon-september-2026 | 2 | ✅ 新页面 |

## 待补充内链

按优先级排列：

### P0 — 消除孤岛

| 来源 | 链向 | 插入位置建议 |
|---|---|---|
| what-is-league-of-legends | what-are-chroma-skins | 介绍个性化/炫彩时 |
| kaisa-prestige-chroma | champion-most | "卡莎在排行榜上的位置"段落 |
| kaisa-prestige-chroma | patch-26-15 | "更多最新臻彩"段落 |
| champion-most-prestige-chromas | kaisa-prestige-chroma | 排行榜中提及卡莎时 |
| champion-most-prestige-chromas | patch-26-15 | "最新新增"段落 |
| patch-26-15-prestige-chromas | champion-most | "对排行榜的影响"段落 |
| patch-26-15-prestige-chromas | kaisa-prestige-chroma | 提到卡莎新臻彩时 |

### P1 — 加强弱链

| 来源 | 链向 | 插入位置建议 |
|---|---|---|
| what-are-chroma-skins | what-are-prestige-chromas | 购买方式段落后补充"臻彩是更高级的变体" |
| what-are-chroma-skins | kaisa-prestige-chroma 或 champion-most | 结尾"看看具体例子" |
| what-are-prestige-chromas | kaisa-prestige-chroma | 展示段落后"以卡莎为例" |
| what-are-prestige-chromas | champion-most | 收藏系统段落"看看谁集得最多" |
| champions-without-prestige-chroma | champion-most | "已有臻彩的英雄排行" |

### P2 — 锦上添花

| 来源 | 链向 | 插入位置建议 |
|---|---|---|
| what-are-prestige-chromas | patch-26-15 | "最新新增"段落 |
| what-are-prestige-chromas | champions-without | "哪些英雄还没有"段落 |
| champions-without-prestige-chroma | kaisa-prestige-chroma | "卡莎的臻彩原画" |
| champions-without-prestige-chroma | patch-26-15 | "最新新增" |

## 内链写法参考

### 英文博客间内链
```astro
<a href={localizedPath(locale, '/blog/what-are-prestige-chromas/')}>prestige chromas</a>
<a href={localizedPath(locale, '/blog/what-are-chroma-skins/')}>regular chroma</a>
<a href={localizedPath(locale, '/blog/champion-most-prestige-chromas/')}>prestige chroma leaderboard</a>
```

### 中文博客间内链
```astro
<a href={localizedPath(locale, '/blog/what-are-prestige-chromas/')}>臻彩</a>
<a href={localizedPath(locale, '/blog/what-are-chroma-skins/')}>炫彩皮肤</a>
<a href={localizedPath(locale, '/blog/champion-most-prestige-chromas/')}>臻彩数量排行榜</a>
```

### FAQ 中引用
```astro
<!-- 英文 -->
<p>See our <a href={localizedPath(locale, '/blog/what-are-prestige-chromas/')}>prestige chroma guide</a> for details.</p>

<!-- 中文 -->
<p>详见我们的<a href={localizedPath(locale, '/blog/what-are-prestige-chromas/')}>臻彩专题</a>。</p>
```

## 新文章内链模板

新增文章时，复制以下模板并填写：

```markdown
### 新文章内链计划

**文章 slug**: your-new-slug
**文章类型**: [入门 / 英雄专题 / 数据分析 / 版本更新 / 概念解释]

**出站链接（正文中至少 2 条）**:
1. → [slug] 锚文字 EN: "..." / ZH: "..."  位置: [段落描述]
2. → [slug] 锚文字 EN: "..." / ZH: "..."  位置: [段落描述]
3. → [slug] 锚文字 EN: "..." / ZH: "..."  位置: [FAQ]

**需要回改的已有文章（至少 2 篇）**:
1. [slug] 插入位置: [段落描述]  锚文字 EN: "..." / ZH: "..."
2. [slug] 插入位置: [段落描述]  锚文字 EN: "..." / ZH: "..."
```

## 更新日志

- 2026-08-07: 初始内链审计，发现 4 个孤岛页面，建立内链矩阵
- 2026-08-07: 新增 top-2-prestige-chroma-champions，从 champion-most 和 patch-26-15 回改内链
- 2026-08-07: 新增 prestige-chroma-summon-august-2026，从 what-are-prestige-chromas 回改官方来源链接
- 2026-08-10: 新增 joy-club-peak-gala-202607，正文中链向 what-are-prestige-chromas，FAQ 中链向 what-are-prestige-chromas
- 2026-08-11: 新增 blue-porcelain-prestige-chromas，正文链向 what-are-prestige-chromas 和 prestige-chroma-summon-august-2026；从 patch-26-15 和 what-are-prestige-chromas 回改内链
- 2026-08-14: 新增 patch-26-16-prestige-chromas，正文链向 blue-porcelain、what-are-prestige-chromas、what-are-chroma-skins、champion-most；从 patch-26-15 和 blue-porcelain 回改内链
- 2026-08-18: 新增 challenger-mayhem-jax-prestige-chroma，正文链向 what-are-prestige-chromas、prestige-chroma-summon-august-2026、splendid-treasure-august-2026
- 2026-08-20: 新增 lucky-gate-porcelain-charm-202608，正文链向 what-are-prestige-chromas、what-are-chroma-skins、blue-porcelain-prestige-chromas、patch-26-16-prestige-chromas、prestige-chroma-summon-august-2026、splendid-treasure-august-2026；从 blue-porcelain-prestige-chromas 和 patch-26-16-prestige-chromas 回改内链
- 2026-08-27: 新增 patch-26-17-prestige-chromas，正文链向 what-are-prestige-chromas、patch-26-16-prestige-chromas、challenger-mayhem-jax-prestige-chroma 和 champion-most-prestige-chromas；按用户要求不修改旧文回链
- 2026-07-29: 新增 lucky-gate-petals-of-spring-chromas-202607，正文链向 what-are-prestige-chromas 和 patch-26-15-prestige-chromas；从 patch-26-15-prestige-chromas 和 lucky-gate-porcelain-charm-202608 回改内链
- 2026-07-01: 新增 joy-club-peak-gala-202606（官方第 202606 期），正文链向原有 joy-club-peak-gala-202607、what-are-prestige-chromas 和 what-are-chroma-skins；从原有 joy-club-peak-gala-202607 和 what-are-prestige-chromas 回改内链
- 2026-08-28: 新增 heartsong-seraphine-prestige-chromas-202608（合并 7 个官网链接的心之歌萨勒芬妮上线资讯），正文链向 what-are-prestige-chromas、patch-26-17-prestige-chromas 与 lucky-gate-porcelain-charm-202608；FAQ 链向 what-are-prestige-chromas；从 patch-26-17-prestige-chromas 与 lucky-gate-porcelain-charm-202608 回改内链
- 2026-09-02: 新增 joy-club-peak-gala-202608（第 202608 期心悦巅峰盛典活动资讯），正文链向 what-are-prestige-chromas（正文+FAQ）、joy-club-peak-gala-202607（上一期）与 full-gift-reward-prestige-chroma-202608（另一消费活动）；从 joy-club-peak-gala-202607（什么是心悦巅峰盛典段落）回链本页
- 2026-09-04: 新增 prestige-chroma-summon-september-2026（2026 年 9 月期璀璨臻彩召唤活动资讯），正文链向 what-are-prestige-chromas（正文+FAQ）与 prestige-chroma-summon-august-2026（往期活动表格）；从 prestige-chroma-summon-august-2026（关于璀璨臻彩召唤活动段落）与 what-are-prestige-chromas（官方获取来源，中英各一条）回链本页；新增共享数据模块 src/domain/brilliant-summon.ts
- 2026-09-10: 新增 `patch-26-18-prestige-chromas`（26.18 版本臻彩资讯），正文链向 what-are-prestige-chromas（正文+FAQ）、patch-26-17-prestige-chromas 与 champion-most-prestige-chromas；从 patch-26-17-prestige-chromas（画廊段落）与 what-are-prestige-chromas（设计理念段落）回链本页
- 2026-09-11: 新增 `canyon-peak-2026-split-2-rewards`（峡谷之巅 2026 第二赛段奖励资讯），正文链向 what-are-prestige-chromas（正文+FAQ）、challenger-mayhem-jax-prestige-chroma（正文，同为赛事/排行奖励）与 joy-club-peak-gala-202608（正文，同窗口另一渠道）；从 challenger-mayhem-jax-prestige-chroma（宝典奖励段落，回链为另一排位/赛事奖励例子）与 patch-26-18-prestige-chromas（设计理念段落，回链为同期最新臻彩资讯）回链本页
- 2026-09-13: 新增 `lucky-gate-ocean-song-202609`（幸运之门·海之歌召唤资讯），正文链向 what-are-prestige-chromas（正文+FAQ）、lucky-gate-porcelain-charm-202608（正文，同系列上一期）、patch-26-17-prestige-chromas（正文，两款珍珠白钻石臻彩出处）与 prestige-chroma-summon-september-2026（正文，并行的另一召唤系列），FAQ 另链 what-are-chroma-skins；从 patch-26-17-prestige-chromas（海之歌段落）与 lucky-gate-porcelain-charm-202608（活动时间段落）回链本页
- 2026-09-23: 新增 `splendid-treasure-september-2026`（华彩秘宝·召唤 9 月场资讯），正文链向 what-are-prestige-chromas（正文+FAQ）、splendid-treasure-august-2026（正文，同系列上一期与往期活动板块表格）、patch-26-18-prestige-chromas（正文，素影霓裳 嫦娥首次出处）与 what-are-chroma-skins（正文，炫彩入门）；华彩秘宝与璀璨臻彩召唤是相互独立的活动系列，正文不链向璀璨臻彩召唤系列，且 `adjacentBlogArticles` 对华彩秘宝文章排除璀璨臻彩召唤系列的相邻推荐；从 what-are-prestige-chromas（召唤活动段落与官方来源列表）与 splendid-treasure-august-2026（下一期预告段落）回链本页
- 2026-09-23: 新增 `patch-26-19-prestige-chromas`（26.19 版本臻彩资讯），正文链向 what-are-prestige-chromas（正文+FAQ）、patch-26-18-prestige-chromas（正文，上一版本）、champion-most-prestige-chromas（正文，排行榜影响）与 lucky-gate-petals-of-spring-chromas-202607（正文，踏雪寻梅往期）；从 patch-26-18-prestige-chromas（画廊段落）与 what-are-prestige-chromas（设计理念段落，中英各一条）回链本页
