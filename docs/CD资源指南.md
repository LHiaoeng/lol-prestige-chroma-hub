...
运行时 PBE 比较使用对应通道根目录的版本元数据：

```text
https://raw.communitydragon.org/{version}/compat-version-metadata.json
```

只接受响应中可校验的 `version` 字段；源站未提供版本值时显示版本缺失，不把 `pbe` 或 `latest` token 冒充具体版本号。
...
- PBE 新增页面在同一本地化视图内并行比较 `pbe` 与 `latest` 的英雄摘要、`skins.json`、皮肤系列、皮肤宇宙和版本元数据；差集键只由实体类型与顶层数字 ID 组成。已有实体的字段、媒体或阶段变化不计入新增，新皮肤的有效阶段只在新增皮肤资料项内展开。
- PBE 比较所需的任一集合或版本元数据失败时，页面保持失败状态并提供重试，不展示部分总数；单个分类为空时显示该分类的空状态，全部分类为空时显示无新增状态。
