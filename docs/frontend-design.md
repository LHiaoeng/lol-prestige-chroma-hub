...
| `/champions/` | PBE 英雄列表 | 全部英雄、头像、皮肤数与阶段任务数 |
| `/champions/{slug}/` | PBE 英雄详情 | 全部皮肤、已下架皮肤、阶段任务皮肤和本站臻彩交叉链接 |
| `/skins/` | PBE 皮肤列表 | 全部皮肤与阶段皮肤的密集索引 |
| `/skins/{slug}/` | PBE 皮肤详情 | 基本信息、聚焦/非聚焦原画、真实动态原画、炫彩和关联实体 |
| `/skinlines/` | PBE 皮肤系列列表 | 系列名称、简介和皮肤数 |
| `/skinlines/{slug}/` | PBE 皮肤系列详情 | 所属宇宙、代表英雄与全部皮肤 |
| `/universes/` | PBE 宇宙列表 | 宇宙图片、简介、系列数与皮肤数 |
| `/universes/{slug}/` | PBE 宇宙详情 | 皮肤系列、代表英雄与全部皮肤 |
...
| `/sitemap.xml` | 搜索引擎入口 | 固定资料页、博客文章和全部 PBE 实体页面索引（不含臻彩详情页） |
...
本节 8.1–8.6 原有约定面向臻彩详情 `/chromas/`。PBE 皮肤详情 `/skins/{slug}/` 与中文路由共用页面实现，采用“单幅主展墙 + 下置展签”的原画优先结构：完整原画先占据最大 `1440px` 宽的主要区域，名称、英雄与档案数据紧随其后。主展墙是唯一的原画媒体入口，不在下方重复渲染静态或动态原画网格。它仅在对应资源存在时渲染静态和动态两枚图标按钮；点击另一按钮切换媒体类型，连续点击当前按钮时在该类型实际存在的聚焦/非聚焦版本间循环。按钮选中态、右上状态标记、标题和屏幕阅读器名称同步反映当前聚焦状态与下一次点击结果。不渲染缺失资源的按钮、面板或占位图；资源在运行时加载失败后立即从可用集合移除，并按动态聚焦 → 动态非聚焦 → 静态聚焦 → 静态非聚焦的顺序回退。页面在有动态资源时默认显示动态，优先聚焦版，切换面板时暂停隐藏的视频，减少动态效果模式下不自动播放。1023px 以下展签由两列改为单列；手机主展墙与后续内容都保留 `--page-gutter` 安全边距。保留稀有度、英雄、ID、基础/已下架状态、阶段母体、简介、系列、宇宙、炫彩、阶段与臻彩对应链接。

主展墙的聚焦/非聚焦静态原画复用 `ImageViewer`，支持键盘打开、Escape 关闭与缩放；主展墙媒体加载失败时不显示占位图，而是移除该媒体并切换到下一可用资源。页面底层优先使用聚焦动态原画（静音自动循环播放），缺少动态资源时改用聚焦原画；有动态原画时静态原画隐藏，仅减少动态效果时回退显示，暗色渐变保证正文可读。主展墙动态原画使用原生视频控件，默认静音自动播放，减少动态效果时保持暂停。手机顶栏在皮肤详情中分为品牌/语言与导航两行，媒体保持完整比例，炫彩两列，关联链接触控高度至少 44px；页面局部尊重减少动态效果。
