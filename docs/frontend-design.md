...
本节 8.1–8.6 原有约定面向臻彩详情 `/chromas/`。PBE 皮肤详情 `/skins/{slug}/` 与中文路由共用页面实现，采用完整原画与展签双栏；1023px 以下顺序为原画、预览提示、标题与常显资料。保留稀有度、英雄、ID、基础/已下架状态、阶段母体、简介、系列、宇宙、炫彩、阶段与臻彩对应链接。

首屏与聚焦/非聚焦原画复用 `ImageViewer`，支持键盘打开、Escape 关闭与缩放；图片失败时回退至本站占位图。页面底层固定使用聚焦动态原画，缺少动态资源时使用聚焦原画；同时保留聚焦原画作为静态底图与减少动态效果时的回退，暗色渐变保证正文可读。动态原画仍使用原生视频控件，不自动播放。手机顶栏在皮肤详情中分为品牌/语言与导航两行，媒体保持完整比例，炫彩两列，关联链接触控高度至少 44px；页面局部尊重减少动态效果。
