...
英雄详情的可见契约只保留英雄名称，以及由顶层皮肤与有效任务阶段组成、按最终皮肤 ID 去重的统一皮肤名称、数据源正方形缩略图和本地化稀有度；顶层记录与 tier ID 冲突时 tier 资料优先，阶段详情保留独立访问能力；皮肤集合默认按发布顺序，可切换为按稀有度排序；不渲染默认外观、称号、简介、背景故事、属性或技能，也不在成功状态显示“资料加载完成”。英雄目录卡片同时呈现头像、名称和定位，头像或定位缺失时使用明确的中英文缺失态。皮肤与阶段链接使用普通 `<a>` 导航，并保留英雄 ID、所属皮肤 ID、阶段 ID 和数据通道；同一页面的英文和简体中文视图不得互相补值。
