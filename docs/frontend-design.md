...
| `/universes/` | 皮肤宇宙目录壳 | 浏览器加载皮肤宇宙列表；详情使用 `/universes/detail/?id={universeId}` |
...
| `/blog/champions-without-prestige-chroma/` | 博客文章 | 基于已提交快照、可手动刷新的未获得臻彩原画英雄清单 |
| `/blog/what-are-chroma-skins/` | 博客文章 | 站点自有的炫彩皮肤科普与完整 FAQ |
| `/404/` | 未找到状态 | 错误说明和返回目录入口 |
| `/sitemap.xml` | 搜索引擎入口 | 固定资料页、博客文章和四个运行时目录壳（不含任何实体详情页） |
...
博客图片可引用公开 CDN，或将文章实际使用的素材保存到 `public/images/blog/<article>/` 以便迁移。图片声明宽高、替代文本、懒加载策略与 `/placeholder.svg` 降级。列表输出 `CollectionPage`，文章输出 `BlogPosting` 与 `BreadcrumbList`，并使用文章类型 Open Graph、绝对分享图地址、发布日期和修改日期。可索引博客文章和静态臻彩详情同时包含更新时间与封面图；CommunityDragon 运行时详情使用 `noindex` 且不进入 Sitemap。
...
`champions-without-prestige-chroma` 是渐进增强例外：Astro 从已提交的 `data/champion-coverage.snapshot.json` 输出可独立索引的双语正文；浏览器再请求相同数据源并刷新数字、说明、名单和头像。普通构建不联网，维护命令会记录两个本地化视图、来源 URL、抓取时间和内容版本；浏览器请求失败则保留构建快照，完整目录 JSON 不进入公开产物。
...
运行时入口只输出静态壳，展示标题、数据通道控制、加载状态和无脚本提示；URL 与请求使用 `pbe`/`latest`，中文界面显示 `PBE`/“正式服”，英文界面显示 `PBE`/`Live`。页面语言决定本地化视图，但不把内部 locale、本地化视图 token 或原始请求地址放进主界面。英雄、皮肤系列和皮肤宇宙的列表与详情分别使用独立页面壳；皮肤不提供全量列表，只从英雄详情进入 `/skins/detail/`。英雄列表对齐参考项目，使用小方形头像、名称和定位组成的可点击网格；定位缺失时显示明确缺失态。英雄详情只展示英雄名称，并以可扫描卡片展示由顶层皮肤与有效任务阶段组成的统一皮肤资料项：顶层项按皮肤 ID 去重，阶段项按所属皮肤 ID 与阶段 ID 去重，使用数据源正方形缩略图和本地化稀有度；tier ID 与顶层皮肤 ID 冲突时 tier 资料优先；皮肤列表默认按发布顺序，并提供按稀有度排序的选择控件；英雄详情成功后不显示完成状态文案。阶段链接保留英雄、所属皮肤和阶段 ID。皮肤详情显示名称、稀有度、描述、所属英雄、系列与宇宙、五类核心媒体和历史版本原画，并始终展示皮肤本体项与真正炫彩的数量、图片和颜色；皮肤本体项不携带炫彩颜色，颜色或名称缺失时保留明确缺失态。阶段详情沿用所属皮肤定位，优先使用阶段自己的名称与媒体，允许继承的文字关系字段只来自同一响应。英文皮肤详情只显示 SkinSpotlights 与 Teemo.GG，中文只显示布锅锅语音站、哔哩哔哩与布锅锅 3D 模型站，外链统一新标签打开并使用安全 `rel`。皮肤系列和皮肤宇宙列表保留各自的搜索、排序与分页。PBE 新增页在同一语言视图比较 PBE 与正式服的英雄、皮肤、皮肤系列和皮肤宇宙，展示来源版本、分类数量与独立空状态；皮肤名称缺失时显示本地化缺失态，仍保留其实体资料链接。英雄、皮肤系列和皮肤宇宙详情使用数字 `id`，皮肤详情额外要求 `champion` 数字提示；详情核心资料成功后再加载皮肤系列或皮肤宇宙关联。详情状态由客户端控制器负责，统一支持加载骨架、结构化失败、重试、取消和迟到响应保护。

运行时详情使用 `noindex`，不生成实体静态 HTML，也不进入 sitemap。旧的 slug 实体 URL 不提供兼容页或跳转；Cloudflare Static Assets 对这些路径返回站点 404。CommunityDragon 直接由浏览器请求，默认 `pbe`，显式 `channel=pbe` 与 `channel=latest` 都可在链接转发时保留；路径、区域、通道和响应契约由领域层统一校验。静态臻彩详情保留完整 SEO 内容，不设置运行时 `noindex`。

臻彩详情是静态优先页面：正文、图片、SEO、主要操作和静态关系不依赖外部源站；正文完成后可补充对应英雄和炫彩所属皮肤资料。补充失败提供轻量重试并保留正文，媒体失败只移除媒体，不改变静态事实。


运行时详情不渲染静态皮肤展墙；图片和关系卡片仅在浏览器请求成功后创建，失败状态保留可访问的重试入口。

英雄详情的可见契约只保留英雄名称，以及由顶层皮肤与有效任务阶段组成的统一皮肤名称、数据源正方形缩略图和本地化稀有度；顶层项按皮肤 ID 去重，阶段项按所属皮肤 ID 与阶段 ID 去重，tier ID 与顶层皮肤 ID 冲突时 tier 资料优先，阶段详情保留独立访问能力；皮肤集合默认按发布顺序，可切换为按稀有度排序；不渲染默认外观、称号、简介、背景故事、属性或技能，也不在成功状态显示“资料加载完成”。英雄目录卡片同时呈现头像、名称和定位，头像或定位缺失时使用明确的中英文缺失态。皮肤与阶段链接使用普通 `<a>` 导航，并保留英雄 ID、所属皮肤 ID、阶段 ID 和数据通道；同一页面的英文和简体中文视图不得互相补值。
...
静态臻彩详情的外部动作当前覆盖 SkinSpotlights、KHADA、Google 和版本公告；运行时皮肤详情按本地化视图分别覆盖 SkinSpotlights/Teemo.GG 或布锅锅语音站/哔哩哔哩/布锅锅 3D 模型站。所有 URL 构造集中在 `src/domain/detail-actions.ts`，组件不拼接业务 URL。
