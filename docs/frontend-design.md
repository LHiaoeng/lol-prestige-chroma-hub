...
`/champions/{slug}/` 与中文路由共用英雄详情模板。首屏展示头像、称号、名称、完整简介及皮肤/阶段/臻彩数量，搭配可进入皮肤详情的基础原画；图片按自身比例完整展示，缺少原画时使用纯资料布局。桌面双栏，1023px 及以下单列，来源说明放在首屏资料之后。后续保留全部皮肤、阶段任务和臻彩关联，分别复用 `PbeSkinCard` 与 `ChromaCard`，臻彩卡片下保留所属皮肤名称。新增图片使用已有失败回退绑定；CommunityDragon 图片不使用腾讯路径，臻彩卡片保留规范源、腾讯源、占位图回退。面包屑与阶段链接触控高度至少 44px，内容无需脚本即可阅读。

### 8.0 PBE 皮肤详情

皮肤名称使用 `var(--gold)`，支持长名称自然换行；稀有度宝石与标题字号同步并对齐第一行。桌面和手机均保留名称相对背景故事的一级字号优势。

皮肤名称字号仅略高于背景故事；背景故事不设置独立的窄行宽上限，完整使用基本信息块宽度。

皮肤基本信息块与后续内容区块等宽。稀有度宝石和皮肤名称共享同一个响应式尺寸变量且不换行，标题使用克制字号；极窄视口的超长名称允许视觉省略，但 DOM 与 `title` 保留完整名称。

皮肤基本信息按“稀有度宝石 + 皮肤名称 → 金色分隔线 → 背景故事 → 次级资料”的顺序合并为一个展签正文块，不再把标题和故事拆成两个区段。英雄链接、基础/阶段关系、炫彩数量、档案编号与来源说明位于故事之后；Legacy 徽章可贴近标题，但不重复出现在故事区。无宝石或无背景故事时不渲染对应元素，也不保留空白占位。

本节 8.1–8.6 原有约定面向臻彩详情 `/chromas/`。PBE 皮肤详情 `/skins/{slug}/` 与中文路由共用页面实现，采用“单幅主展墙 + 下置展签”的原画优先结构：完整原画先占据最大 `1440px` 宽的主要区域，名称、英雄与档案数据紧随其后。主展墙是唯一的原画媒体入口，不在下方重复渲染静态或动态原画网格；墙内只显示图片或视频，不显示可见标题、状态说明、操作提示或装饰。静态、动态切换按钮与下载按钮另起一行组成居中的底部控制条；媒体按钮仅在对应资源存在时渲染，点击另一按钮切换媒体类型，连续点击当前按钮时在该类型实际存在的聚焦/非聚焦版本间循环。媒体按钮的选中态、右上状态标记和屏幕阅读器名称同步反映当前聚焦状态与下一次点击结果。下载按钮使用原生 `<details>` 向上展开所有实际存在的资源，当前主展项使用金色选中态和 `aria-current` 高亮；下载名按网页语言使用对应皮肤名称与状态词，找不到匹配语言时默认英文，并保留媒体类型与真实扩展名。主展墙宽度由内容宽度与扣除 Header、面包屑和控制条后的视口高度共同限制，保证主展墙和按钮组完整显示在首屏内。不渲染缺失资源的按钮、面板或占位图；资源在运行时加载失败后立即从可用集合及下载列表移除，并按动态聚焦 → 动态非聚焦 → 静态聚焦 → 静态非聚焦的顺序回退。页面在有动态资源时默认显示动态，优先聚焦版，切换面板时暂停隐藏的视频，减少动态效果模式下不自动播放。1023px 以下展签由两列改为单列；手机主展墙与后续内容都保留 `--page-gutter` 安全边距。保留稀有度、英雄、ID、基础/已下架状态、阶段母体、简介、系列、宇宙、炫彩、阶段与臻彩对应链接。

主展墙的聚焦/非聚焦静态原画复用 `ImageViewer`，支持键盘打开、Escape 关闭与缩放；主展墙媒体加载失败时不显示占位图，而是移除该媒体并切换到下一可用资源。页面底层优先使用聚焦动态原画（静音自动循环播放），缺少动态资源时改用聚焦原画；有动态原画时静态原画隐藏，仅减少动态效果时回退显示，暗色渐变保证正文可读。主展墙动态原画不显示原生视频控制条，默认静音自动循环播放，减少动态效果时保持暂停。手机顶栏在皮肤详情中分为品牌/语言与导航两行，媒体保持完整比例，炫彩两列，关联链接触控高度至少 44px；页面局部尊重减少动态效果。

下载菜单打开后，焦点自动移到带 `aria-current="true"` 的当前主展项；Escape 关闭菜单并把焦点还给下载按钮，点击菜单外部同样关闭。

皮肤展签的稀有度徽章按当前页面语言读取对应 CommunityDragon 区域数据。英文页面使用 `global/default` 的 `rarity`，并将 `kUltimate`、`kMythic`、`kLegendary`、`kEpic`、`kExalted` 与 `kTranscendent` 映射到全球版宝石图标。中文页面使用 `global/zh_cn` 的 `regionRarityId`：4–11 分别映射为史诗、传说、未知、限定、神话、终极、圣堂、卓越，对应 `rarity-gem-icons/cn-gem-{id}.png`；仅在区域字段缺失或无法识别时回退到全球 `rarity`。因此同一款皮肤在国服可以是“限定”，同时在全球分类中仍是 Epic，两者不得互相覆盖。`isLegacy` 仅继续展示为“典藏 / Legacy”的可获取性标记，不代替国服的“限定”稀有度。
