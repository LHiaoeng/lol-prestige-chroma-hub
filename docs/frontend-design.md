...
英雄详情的可见契约只保留英雄名称、`isBase: true` 默认外观原画，以及全部皮肤和有效阶段的名称、缩略图和本地化稀有度；不渲染称号、简介、背景故事、属性或技能。英雄目录卡片同时呈现头像、名称和定位，头像或定位缺失时使用明确的中英文缺失态。皮肤与阶段链接使用普通 `<a>` 导航，并保留英雄 ID、所属皮肤 ID、阶段 ID 和数据通道；同一页面的英文和简体中文视图不得互相补值。
