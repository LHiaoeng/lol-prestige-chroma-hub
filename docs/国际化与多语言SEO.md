...
英文是默认语言，保留 `/`、`/blog/`、`/chromas/<slug>/` 等现有路径。简体中文使用 `/zh-cn/`、`/zh-cn/blog/` 和 `/zh-cn/chromas/<slug>/`。详情页同时保留基于 `skinId` 的中英文兼容入口，例如 `/chromas/147063/` 与 `/zh-cn/chromas/147063/`；数字入口的 canonical、hreflang、站内链接和 sitemap 始终指向可读 slug 主地址。不要创建含义不明确的 `/zh/`。
...
- `src/seo/sitemap.ts`：双语 URL、图片与 XHTML alternate。
...
Sitemap 为每种语言分别输出 `<url>`，并在每个条目中重复相同的 XHTML alternate 集合。详情图和博客封面标题按当前条目语言生成。
...
5. 将页面加入 sitemap 的 canonical pathname 集合；
...
发布前抽查英文与简体首页、博客列表、文章、详情、About、Privacy、404 与 sitemap，确认 canonical 自引用、alternate 双向、站内链接保持当前语言，且构建产物不包含完整 `prestige-chromas.json`。
