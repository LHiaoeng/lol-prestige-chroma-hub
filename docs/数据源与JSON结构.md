...
CommunityDragon 运行时资源的通用规则，以及英雄联盟和云顶之弈的资源入口，见 [CommunityDragon 资源指南](CD资源指南.md)；本文仍只定义本站臻彩目录的数据源和 JSON 契约。
...
英文 `/blog/champions-without-prestige-chroma/` 与简体中文 `/zh-cn/blog/champions-without-prestige-chroma/` 使用两类数据计算尚未获得臻彩原画的英雄：
