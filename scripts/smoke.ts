...
await request(`${base}/`);
await request(`${base}/api/chromas?pageSize=1`);
...
for (const path of ['/data/prestige-chromas.json', '/api/export', '/chromas.json']) { const response = await fetch(`${base}${path}`); if (response.status !== 404) throw new Error(`Sensitive path must return 404: ${path} returned ${response.status}`); }
