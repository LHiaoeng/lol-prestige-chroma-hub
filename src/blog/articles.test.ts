...
describe('blog article metadata', () => {
  ...
  it('publishes twenty bilingual articles with unique canonical routes', () => {
    expect(blogArticles).toHaveLength(20);
    ...
    expect(blogArticles[12].publishedAt).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    expect(blogArticles[12].readingMinutes).toBeGreaterThan(0);
    expect(new Set(blogArticles.map((article) => article.href)).size).toBe(blogArticles.length);
    expect(blogArticles[10].slug).toBe('top-2-prestige-chroma-champions');
    expect(`${blogArticles[13].titleEn} ${blogArticles[13].titleZh}`).not.toMatch(/\d+\.\d+/);
    expect(`${blogArticles[13].summaryEn} ${blogArticles[13].summaryZh}`).not.toMatch(/Ahri|阿狸|17/);
