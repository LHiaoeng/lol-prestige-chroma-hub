...
describe('blog article metadata', () => {
  ...
  it('publishes twenty-one bilingual articles with unique canonical routes', () => {
    expect(blogArticles).toHaveLength(21);
    ...
    expect(blogArticles[13].publishedAt).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    expect(blogArticles[13].readingMinutes).toBeGreaterThan(0);
    expect(new Set(blogArticles.map((article) => article.href)).size).toBe(blogArticles.length);
    expect(blogArticles[11].slug).toBe('top-2-prestige-chroma-champions');
    expect(`${blogArticles[14].titleEn} ${blogArticles[14].titleZh}`).not.toMatch(/\d+\.\d+/);
    expect(`${blogArticles[14].summaryEn} ${blogArticles[14].summaryZh}`).not.toMatch(/Ahri|阿狸|17/);
