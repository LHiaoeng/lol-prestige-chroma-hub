...
describe('blog article metadata', () => {
  ...
  it('publishes nine bilingual articles with unique canonical routes', () => {
    ...
    expect(blogArticles[1].slug).toBe('top-2-prestige-chroma-champions');
    expect(`${blogArticles[3].titleEn} ${blogArticles[3].titleZh}`).not.toMatch(/\d+\.\d+/);
    expect(`${blogArticles[3].summaryEn} ${blogArticles[3].summaryZh}`).not.toMatch(/Ahri|阿狸|17/);
