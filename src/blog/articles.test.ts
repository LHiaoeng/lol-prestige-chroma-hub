...
describe('blog article metadata', () => {
  ...
  it('publishes twenty-three bilingual articles with unique canonical routes', () => {
    ...
    expect(blogArticles[13].slug).toBe('top-2-prestige-chroma-champions');
    expect(`${blogArticles[16].titleEn} ${blogArticles[16].titleZh}`).not.toMatch(/\d+\.\d+/);
    expect(`${blogArticles[16].summaryEn} ${blogArticles[16].summaryZh}`).not.toMatch(/Ahri|阿狸|17/);
