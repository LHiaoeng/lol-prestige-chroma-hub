...
describe('blog article metadata', () => {
  ...
  it('publishes eighteen bilingual articles with unique canonical routes', () => {
    ...
    expect(blogArticles[10].publishedAt).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    expect(blogArticles[10].readingMinutes).toBeGreaterThan(0);
    expect(new Set(blogArticles.map((article) => article.href)).size).toBe(blogArticles.length);
    expect(blogArticles[8].slug).toBe('top-2-prestige-chroma-champions');
    expect(`${blogArticles[11].titleEn} ${blogArticles[11].titleZh}`).not.toMatch(/\d+\.\d+/);
    expect(`${blogArticles[11].summaryEn} ${blogArticles[11].summaryZh}`).not.toMatch(/Ahri|阿狸|17/);
