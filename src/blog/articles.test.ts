...
describe('blog article metadata', () => {
  ...
  it('publishes thirteen bilingual articles with unique canonical routes', () => {
    expect(blogArticles).toHaveLength(13);
    ...
    expect(blogArticles[4].publishedAt).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    expect(blogArticles[4].readingMinutes).toBeGreaterThan(0);
    expect(new Set(blogArticles.map((article) => article.href)).size).toBe(blogArticles.length);
    expect(blogArticles[5].slug).toBe('top-2-prestige-chroma-champions');
    expect(`${blogArticles[7].titleEn} ${blogArticles[7].titleZh}`).not.toMatch(/\d+\.\d+/);
    expect(`${blogArticles[7].summaryEn} ${blogArticles[7].summaryZh}`).not.toMatch(/Ahri|阿狸|17/);
